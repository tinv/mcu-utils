...
#ifndef MU_GPIO_TARGET_H
#define MU_GPIO_TARGET_H
...
#define MU_IO_STATIC_INPUT(ID, DTLABEL)                                                            \
	{.config = {.id = ID,                                                                      \
	 ...
	 .data = {.state = 0, .old_state = 0, .filtered_state = 0}}
#else
#define MU_IO_STATIC_INPUT(ID, DTLABEL)                                                            \
	{.config = {.id = ID,                                                                      \
		    .name = xstr(ID),                                                              \
...
#define MU_IO_STATIC_INPUT_HANDLER(ID, DTLABEL, HANDLER, FLAGS)                                    \
	{.config = {.id = ID,                                                                      \
		    ...
		    .flags = FLAGS,                                                                \
		    .filter_num_stable_val = 0U},                                                  \
	 .data = {.state = 0, .old_state = 0, .cb_data = {0}, .filtered_state = 0}}
	...
	{                                                                                          \
		.config =                                                                          \
			{                                                                          \
				.id = ID,                                                          \
				.name = xstr(ID),                                                  \
				.dsc = (const char *)DT_PROP_OR(DT_NODELABEL(DTLABEL), label,      \
								"N/A label"),                      \
				.dt = GPIO_DT_SPEC_GET_OR(DT_NODELABEL(DTLABEL), gpios, {0}),      \
			},                                                                         \
		.data = {                                                                          \
